...
const Formulario = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        fetchData(decodedToken.id); // Carga los datos del usuario al iniciar
      } catch (error) {
  ...
  // Carga los datos del usuario cuando inicia sesión
  const fetchData = async (userId) => {
    try {
      const token = Cookies.get('token');
      const response = await axios.get(`http://localhost:4000/api/getoneBD/${userId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:4000/api/updateBD/${userId}`, dataToSend, {
        withCredentials: true,
        headers: {
          'Authorization': `Bearer ${token}`
      ...
      if (error.response && error.response.status === 403) {
        ...
        setErrorMessage("Error 404: Datos no encontrados.");
      } else {
